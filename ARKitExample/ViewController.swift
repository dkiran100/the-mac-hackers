...
class ViewController: UIViewController {
    ...
    func addheart() {
        heart.loadModel()
        heart.position = kStartingPosition
        heart.rotation = SCNVector4Zero
        sceneView.scene.rootNode.addChildNode(heart)
        heart.geometry?.firstMaterial?.diffuse.contents = "heart_cm_texture.png"
    ...
    private func deltas() -> (sin: CGFloat, cos: CGFloat) {
        return (sin: kMovingLengthPerLoop * CGFloat(sin(heart.eulerAngles.y)), cos: kMovingLengthPerLoop * CGFloat(cos(heart.eulerAngles.y)))
    }
}
